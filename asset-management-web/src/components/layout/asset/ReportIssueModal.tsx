...
import { reportIssueApi } from "../../../services/issue.service";
...
export default function ReportIssueModal({ isOpen, asset, onClose, onSuccess }: ReportIssueModalProps) {
  ...
  } = useUserForm<ReportIssueFormValues>({
    schema: reportIssueSchema,
    defaultValues: {
      issueTitle: "MALFUNCTION",
      remark: "",
    },
    onSubmit: async (data) => {
      if (!asset) throw new Error("No asset selected");
      return reportIssueApi(asset.id, data);
    },
    onSuccess: (issue) => {
      onSuccess(issue);
      onClose();
  ...
  useEffect(() => {
    if (isOpen) {
      reset({ issueTitle: "MALFUNCTION", remark: "" });
    }
  }, [isOpen, reset]);

  if (!isOpen || !asset) return null;

  const currentIssueTitle = watch("issueTitle");

  const issueOptions = IssueTitleEnumValues.map((val) => ({
      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="w-full flex items-center justify-center mb-2 pt-1">
          ...
          <div className="flex flex-col text-left">
            <h3 className="text-xl mt-2 font-bold tracking-tight text-[var(--text-main)] leading-none">Report Issue</h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 text-left">
          <div className="border border-[var(--border-color)]/30 rounded-2xl p-4 bg-[var(--surface-hover)]/5 space-y-5">
            <div className="px-1 flex items-center gap-2">
              <AlertCircle size={16} className="text-[var(--color-growth-green)]" />
              <h3 className="text-sm font-bold text-[var(--text-main)]">Issue Details</h3>
            </div>

            <p className="text-xs text-[var(--text-muted)] px-1 leading-relaxed">
              Reporting a concern for <span className="font-bold text-[var(--text-main)]">{asset.deviceName}</span> ({asset.assetTag}).
            </p>
