...
import type { UpdateEmployeeFormValues, EmployeeDto } from "../../../types/employee.types";
import { updateEmployeeSchema } from "../../../types/employee.types";
...
interface EditEmployeeModalProps {
  ...
  onClose: () => void;
  onUpdated: (emp: EmployeeDto) => void;
}

export default function EditEmployeeModal({ isOpen, employee, onClose, onUpdated }: EditEmployeeModalProps) {
  ...
  const {
    ...
    handleClose,
    handleSubmit,
  } = useUserForm<UpdateEmployeeFormValues>({
    ...
    onSubmit: (data) => updateEmployeeApi(employee!.id, data, selectedFile, isImageDeleted),
    onSuccess: onUpdated,
    onClose,
  ...
  const {
    ...
    isDragging,
    dragProps,
  } = useImageUpload({ aspect: 1, initialImage: employee?.image });
  ...
  useEffect(() => {
    if (employee && isOpen) {
      ...
      resetImage(employee.image);
    }
  }, [employee, isOpen, reset, resetImage]);

  if (!isOpen || !employee) return null;

  return (
    <Modal isOpen={isOpen} onClose={handleClose} maxWidth="max-w-[520px]">
      <div className="flex flex-col gap-4">
        {/* Header - Logo on Right */}
        <div className="flex items-center justify-between mb-10 w-full px-2">
          <div className="flex flex-col gap-1">
            <h3 className="text-lg font-bold tracking-tight text-[var(--text-main)] leading-none">Employee Details</h3>
            <p className="text-xs text-[var(--text-muted)] font-medium">View and update employee information</p>
          </div>
          ...
          <div className="space-y-6 px-2">
            ...
            <div className="space-y-1">
              <div className="pt-2 mb-2">
                <h3 className="text-sm font-bold text-[var(--text-main)]">Employee Information</h3>
